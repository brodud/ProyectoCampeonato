...
class PaisDAO {
    ...
    public function insertar() {
        $sql = "INSERT INTO g8_pais (nombre, sigla_fifa, bandera_url, id_confederacion)
                VALUES ('" . $this->nombre . "', 
                        '" . $this->sigla_fifa . "', 
                        '" . $this->bandera_url . "', 
    ...
    public function consultarNoParticipantes($id_campeonato) {
        return "SELECT p.id_pais, p.nombre, p.sigla_fifa, p.bandera_url,
                ...
                WHERE p.id_pais NOT IN (
                    SELECT id_pais FROM g8_participante WHERE id_campeonato = '" . $id_campeonato . "'
                )
                ORDER BY c.nombre, p.nombre";
    }
